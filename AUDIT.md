...
- **§5.3.1 LLM match adjudication — implemented.** `POST /api/matches/adjudicate` batches conflict/code-only groups to the Anthropic Messages API (Claude Haiku by default, `ADJUDICATE_MODEL`-configurable; called from the backend via stdlib urllib, no added dependency, gated on `ANTHROPIC_API_KEY`). Verdicts land in an advisory `match_suggestions` table (never affect matching until confirmed); the Match-review UI shows them per group — a `same` verdict gets a one-click **Accept AI · confirm**, `different`/`uncertain` are flagged for the human. Idempotent + progressive (already-suggested groups skipped), and a human review deletes the suggestion so the human verdict always wins. Note: in this deployment EAN capture proved unavailable (Public/Plaisio listings and PDPs expose no gtin; Kotsovolos PDPs are JS-rendered), so model-code matching + this adjudicator carry the matching layer — which is the exact long-tail case §5.3.1 was meant for.
- **§2.1–2.3 EAN/MPN-anchored server-side matching — implemented.** `products.ean`/`products.mpn` (indexed, auto-migrated); all three PLP fetchers capture identity fields (Plaisio schema.org `gtin*`/`mpn`, Kotsovolos `mfPartNumber` + attribute scan, Public sku-JSON field probe) and the runner gains `--enrich-ids` (PDP JSON-LD pass via `/api/ingest/missing-ids` → `/api/ingest/ids`). Server-side matcher (union-find; priority override → EAN → MPN → normalized code → fuzzy containment; Python port of the frontend code extractor) writes a `product_matches` table, rebuilt lazily after ingest/retire. `GET /api/matches?status=` + `POST /api/matches/review`; `match_overrides` pair verdicts (same/different) always win recomputation — verified by test. Match-review UI page with side-by-side candidates, Confirm/Not-the-same, needs-review nav badge. Client-side matching remains the fallback for all views (switching views to server matches is the follow-up).
